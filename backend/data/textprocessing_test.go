...
func TestGetWordFrequencies(t *testing.T) {
	...
	frequencies := GetWordFrequencies(text, 3, 10, false, nil)
...
func TestGetWordFrequenciesMinLength(t *testing.T) {
	...
	frequencies := GetWordFrequencies(text, 4, 10, false, nil)
...
func TestGetWordFrequenciesTopN(t *testing.T) {
	...
	frequencies := GetWordFrequencies(text, 1, 5, false, nil)
...
func TestGetWordFrequenciesOnlyStopwords(t *testing.T) {
	...
	frequencies := GetWordFrequencies(text, 2, 10, false, nil)
...
func TestGetWordFrequenciesCustomExcluded(t *testing.T) {
	text := "machine learning deep learning neural networks machine learning"

	// Exclude "learning" and "machine"
	customExcluded := []string{"learning", "machine"}
	frequencies := GetWordFrequencies(text, 3, 10, false, customExcluded)

	// "learning" and "machine" should be excluded
	for _, freq := range frequencies {
		if freq.Word == "learning" || freq.Word == "machine" {
			t.Errorf("Custom excluded word '%s' should have been filtered out", freq.Word)
		}
	}

	// "deep", "neural", "networks" should be present
	words := make(map[string]bool)
	for _, freq := range frequencies {
		words[freq.Word] = true
	}
	...
	if !words["deep"] {
		t.Error("Expected 'deep' in results")
	}
	if !words["neural"] {
		t.Error("Expected 'neural' in results")
	}
	if !words["networks"] {
		t.Error("Expected 'networks' in results")
	}
}
