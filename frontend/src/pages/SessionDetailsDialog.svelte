<script>
      ...
      itemsWithStartNonNull.forEach(({ it, start }) => {
        ...
        const dur =
          it.DurationMinutes != null
            ...
            : parseDurationToMinutes(it.Duration);
  ...
  let omittedToggleLabels = $derived(
    (sessionAggregates || []).reduce((m, sa) => {
...
<Modal bind:open size="lg" dismissable={false} class="session-dialog">
              ...
              <div class="flex items-center gap-3 mt-2">
                {#if sa.locations.length}
                  <p class="text-xs text-gray-600 dark:text-gray-400">
                    Location: {sa.locations.join(', ')}
                  </p>
                {/if}

                {#if sa.rooms.length}
                  <p class="text-xs text-gray-600 dark:text-gray-400">
                    Room(s): {sa.rooms.join(', ')}
                  </p>
                {/if}

                {#if sa.speakers.length}
                  <p class="text-xs text-gray-600 dark:text-gray-400">
                    Speakers: {sa.speakers.join(', ')}
                  </p>
                {/if}
              </div>
            </div>
            ...
            <div class="mt-3">
                  ...
                  {#each getDisplayedItems(sa) as it}
                    <li class="py-2">
                      ...
                      <div class="text-xs text-gray-500 dark:text-gray-400">
                        {it.StartDate ? it.StartDate : it.Start ? it.Start : ''}
                        {#if it.Duration}
                          — {it.Duration}{/if}
                        {#if it.Room}
                          — {it.Room}{/if}
              ...
              {:else}
                <p class="text-xs text-gray-500 dark:text-gray-400">
                  No contributions displayed. Expand omitted to inspect items without start times.
                </p>
              {/if}
            </div>
          {/if}
          ...
          {#if sa.excludedCount > 0}
            <div class="mt-2">
              <button
                ...
                type="button"
                onclick={() => toggleExpanded(sa.title)}
              >
