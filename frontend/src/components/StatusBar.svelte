<script>
  ...
  onDestroy(() => {
    ...
    try { if (_clockTimer) clearInterval(_clockTimer); } catch (e) { /* ignore */ }
  });
...
<div class="fixed bottom-0 left-0 right-0 h-10 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-700 text-sm text-gray-700 dark:text-gray-300 flex items-center justify-between z-40 shadow-md dark:shadow-black/40"
    ...
    {:else}
      ...
      <div class="flex items-center gap-2 pl-3 justify-end min-w-2xs">
        <span class="text-sm font-mono text-gray-500 dark:text-gray-400" title={new Date().toString()} aria-label="Current date and time">{currentTime}</span>
      </div>
    {/if}
   </div>
